...
public class AnnonController {
    ...
    @PreAuthorize("hasRole('SUPERADMIN')")
    @GetMapping("/delete")
    public String delete(@RequestParam Long bno, RedirectAttributes redirectAttributes) {
        // bno가 null이 아니고 유효한 값인지 검증 (선택 사항)
        if (bno == null || bno <= 0) {
            throw new IllegalArgumentException("유효하지 않은 게시글 번호입니다.");
        }

        // 삭제 로직 추가
        annonService.delete(bno); // 실제 삭제 메서드 호출

         redirectAttributes.addFlashAttribute("message", "게시글이 성공적으로 삭제되었습니다.");

        return "redirect:/annon/main"; // 리다이렉트
    }
